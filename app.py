...
def index():
    try:
        ...
        # Get the most recent scan based on tab selection
        if tab == 'gems':
            # Brand Hunter gems only
            latest_scan = ProductScan.query.filter_by(scan_type='brand_hunter').order_by(desc(ProductScan.scan_date)).first()
        else:
            # All products (general scans)
            latest_scan = ProductScan.query.filter_by(scan_type='general').order_by(desc(ProductScan.scan_date)).first()
