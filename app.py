...
def refresh_images():
    ...
    try:
        ...
        stats = refresh_product_images(batch_size=10, delay=0.5)
        return jsonify({
            'success': True,
            'stats': stats,
...
def refresh_sellers():
    ...
    try:
        ...
        # Count products with seller_id
        products_with_seller = Product.query.filter(
        ...
        if not products_with_seller:
            return jsonify({
                'success': False,
                'message': 'No products have seller_id set. Run /api/scan first.',
                'total_products': total,
                'debug': 'No seller_ids found in database'
            })
        
        # Find unique sellers needing names
        sellers_needing_names = set()
        sellers_with_names = set()
        for p in products_with_seller:
            if p.seller_name and p.seller_name.strip() and p.seller_name not in ['TikTok Shop', 'Unknown Seller', 'Unknown']:
                sellers_with_names.add(p.seller_id)
            else:
                sellers_needing_names.add(p.seller_id)
        
        if not sellers_needing_names:
            return jsonify({
                'success': True,
                'message': 'All sellers already have names!',
                'stats': {
                    'total_products': total,
                    'products_with_seller_id': len(products_with_seller),
                    'unique_sellers_with_names': len(sellers_with_names),
                    'sellers_needing_names': 0
                }
            })
        
        stats = refresh_seller_names(delay=0.3)
        return jsonify({
            'success': True,
            'stats': stats,
            'message': f"Found {stats['sellers_found']}/{stats['unique_sellers']} sellers, updated {stats['products_updated']} products"
        })
    except Exception as e:
        import traceback
        error_trace = traceback.format_exc()
