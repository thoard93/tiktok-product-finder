...
def get_brand_products(seller_id):
    ...
    Query params:
    - max_influencers: Max influencer count (default: 100)
            ...
            for page in range(1, pages + 1):
                response = requests.get(
                    ...
                    params={
                        'seller_id': seller_id,
                        'page_num': page,
                        'page_size': 20,  # More products
                        'seller_product_sort_field': 5,  # Influencers
                        'sort_type': 2,  # Ascending (lowest first)
                    },
                    auth=auth,
                    timeout=30
                )
        ...
        for page in range(1, 6):  # Search 5 pages
            response = requests.get(
                ...
                params={
                    ...
                    'sort_type': 1,  # Descending (top brands first)
                },
                auth=auth,
                timeout=30
            )
...
def brand_hunter_full_scan():
    ...
    - max_brands: Max brands to scan per run (default: 5)
    - pages_per_brand: Pages to scan per brand (default: 2)
    """
    import time
    ...
    try:
        ...
        max_brands = request.args.get('max_brands', 5, type=int)  # Reduced default
        pages_per_brand = request.args.get('pages_per_brand', 2, type=int)
        ...
        # Skip discovery if parameter set (saves time)
        skip_discovery = request.args.get('skip_discovery', 'false').lower() == 'true'
        
        discovered_brands = 0
        newly_followed = 0
        
        # Step 1: Discover and follow top brands (3 pages) - skip if requested
        if not skip_discovery:
            for page in range(1, 2):  # Reduced to just 1 page
                try:
                    response = requests.get(
                        'https://open.echotik.live/api/v3/echotik/seller/list',
                        params={
        ...
        for brand in followed_brands:
            try:
                for page in range(1, pages_per_brand + 1):
                    response = requests.get(
                        ...
                        params={
                            ...
                            'page_size': 20,  # More products
                            'seller_product_sort_field': 5,  # Influencers
                            'sort_type': 2,  # Ascending (low influencers first)
                        },
                        auth=auth,
                        timeout=30
                    )
                ...
                brand.last_scanned = datetime.utcnow()
                db.session.commit()  # Commit after each brand to save progress
                brands_scanned += 1
        ...
        return jsonify({
            ...
            'message': f'Full scan complete! {total_new} new gems from {brands_scanned} brands in {elapsed:.1f}s.'
        })
    except Exception as e:
        import traceback
...
@app.route('/api/brands/scan-next')
def scan_next_brand():
    """
    Scan just the next unscanned brand.
    Sorts by 30-DAY SALES DESCENDING to find products that are actually selling!
    Only keeps products with 1-100 influencers (the sweet spot).
    """
    try:
        max_influencers = request.args.get('max_influencers', 100, type=int)
        min_sales = request.args.get('min_sales', 1, type=int)  # Require at least 1 sale
        pages = request.args.get('pages', 10, type=int)  # Scan up to 10 pages
        start_page = request.args.get('start_page', 1, type=int)  # Allow starting from later pages
        ...
        # Find brand that needs scanning (oldest last_scanned or never scanned)
        brand = Brand.query.filter(Brand.is_followed == True).order_by(
        ...
        products_list = []
        skipped_no_sales = 0
        skipped_high_influencers = 0
        
        for page in range(start_page, start_page + pages):
            try:
                # Sort by 30-DAY SALES DESCENDING - gets products that are selling!
                # Then we filter for low influencers to find the hidden gems
                response = requests.get(
                    'https://open.echotik.live/api/v3/echotik/seller/product/list',
                    params={
                        'seller_id': brand.seller_id,
                        'page_num': page,
                        'page_size': 20,
                        'seller_product_sort_field': 4,  # 4 = 30-day sales
                        'sort_type': 1,  # 1 = Descending (highest sales first)
                    },
                    auth=auth,
                    timeout=30
                )
                ...
                for p in products:
                    ...
                    if influencer_count > max_influencers:
                        skipped_high_influencers += 1
                        continue
                    if influencer_count < 1:
                        continue
                    
                    # Check 30-day sales - skip dead products!
                    sales_30d = int(p.get('total_sale_30d_cnt', 0) or 0)
        ...
        return jsonify({
            'success': True,
            'brand': brand.seller_name,
            'brand_id': brand.seller_id,
            'pages_scanned': f'{start_page}-{start_page + pages - 1}',
            'new_products': new_products,
            'updated_products': updated_products,
            'skipped_no_sales': skipped_no_sales,
            'skipped_high_influencers': skipped_high_influencers,
            'products': products_list[:10],
            'message': f'Scanned {brand.seller_name}: {new_products} new gems found, {skipped_high_influencers} skipped (high influencers), {skipped_no_sales} skipped (no sales)'
        })
...
@app.route('/api/brands/<seller_id>/deep-scan')
def deep_scan_brand(seller_id):
    """
    Deep scan a specific brand - goes through many pages to find ALL hidden gems.
    Use this for big brands like QVC with 60K+ products.
    
    Parameters:
    - start_page: Which page to start from (default: 1)
    - pages: How many pages to scan (default: 50)
    - max_influencers: Max influencer count to include (default: 100)
    - min_sales: Minimum 30d sales to include (default: 1)
    """
    try:
        ...
        brand = Brand.query.filter_by(seller_id=seller_id).first()
        brand_name = brand.seller_name if brand else seller_id
        ...
        for page in range(start_page, start_page + pages):
            try:
                # Sort by 30-DAY SALES DESCENDING
                response = requests.get(
                    'https://open.echotik.live/api/v3/echotik/seller/product/list',
                    params={
                        'seller_id': seller_id,
                        'page_num': page,
                        'page_size': 20,
                        'seller_product_sort_field': 4,  # 30d sales
                        'sort_type': 1,  # Descending
                    },
                    auth=auth,
                    timeout=30
        ...
        return jsonify({
            ...
            'brand': brand_name,
            'brand_id': seller_id,
            'pages_scanned': f'{start_page}-{start_page + pages_with_data - 1}',
            'total_pages_with_data': pages_with_data,
            'new_products': new_products,
            'updated_products': updated_products,
            'skipped_high_influencers': skipped_high_influencers,
            'skipped_no_sales': skipped_no_sales,
            'gems_found': products_list[:20],
            'message': f'Deep scanned {brand_name}: Found {new_products} new gems!'
        })
...
@app.route('/api/brands/<seller_id>/debug')
def debug_brand_products(seller_id):
    """
    Debug endpoint: See raw API data for a brand's products.
    Shows why products might not be qualifying.
    """
    try:
        auth = HTTPBasicAuth(
            os.environ.get('ECHOTIK_USERNAME'),
        ...
        analysis = []
        for p in products:
            influencers = int(p.get('total_ifl_cnt', 0) or 0)
            commission = float(p.get('product_commission_rate', 0) or 0)
            
            reasons = []
            if influencers < 1:
                reasons.append(f'influencers={influencers} (need >=1)')
            if influencers > 50:
                reasons.append(f'influencers={influencers} (need <=50)')
            if commission <= 0:
                reasons.append(f'commission={commission} (need >0)')
            
            analysis.append({
                'product_id': p.get('product_id'),
                'name': p.get('product_name', '')[:50],
                'influencers': influencers,
                'commission_raw': p.get('product_commission_rate'),
                'commission_percent': commission * 100 if commission < 1 else commission,
                'gmv': p.get('total_sale_gmv_amt'),
        ...
        return jsonify({
            'success': True,
            'brand': brand.seller_name if brand else 'Unknown',
            'seller_id': seller_id,
            'total_products_returned': len(products),
            'qualifying_products': sum(1 for a in analysis if a['qualifies']),
            'products': analysis,
