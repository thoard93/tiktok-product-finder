...
def scan_deals():
    ...
    try:
        ...
        print(f"🎁 Starting Deal Hunter scan: {pages} pages, max {max_videos} vids")
        
        # Helper function to save deal product
        def save_deal_product(p):
            try:
                product_id = p.get('product_id', '')
                if not product_id:
                    return False
                ...
                if existing:
                    ...
                    existing.video_7d = int(p.get('total_video_7d_cnt', 0) or 0)
                    existing.sales_7d = int(p.get('total_sale_7d_cnt', 0) or 0)
                    existing.has_free_shipping = True
                    existing.last_updated = datetime.utcnow()
                    return True # Count as saved/updated
                else:
                    # Create new
                    product = Product(
                        product_id=product_id,
                ...
                print(f"Error saving deal product: {e}")
                return False

        for page in range(1, pages + 1):
            if page % 5 == 0:
                print(f"Scanning deal page {page}...")
                
            # Use search_deal_products helper
            search_res = search_deal_products(
                page=page, 
                page_size=20, 
                max_videos=max_videos, 
                min_sales_7d=min_sales_7d
        ...
        import traceback
        set_scan_status(False) # Force unlock
        print(f"Deal scan error: {e}")
