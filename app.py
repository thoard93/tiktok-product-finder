...
def verify_product_status(product_id, auth):
    ...
    try:
        ...
        response = requests.get(
            ...
            params={'product_id': product_id, 'region': 'US'},
            auth=auth,
            timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            # Check if product has no sales data recently (might indicate issues)
            # But it's still "active" if we got data back
...
@app.route('/')
def dashboard():
    """Main dashboard page - serves mobile or desktop version"""
    # Check for forced desktop mode
    if request.args.get('desktop') == '1':
        return render_template('dashboard_v3.html')
    
    # Check if mobile and mobile template exists
    if is_mobile():
        try:
...
def get_stats():
    ...
    try:
        ...
        stats = {
            'total_products': available_products,  # Only count available by default
            'total_all': total_products,  # Including unavailable
        ...
        # Base filter for available products only
        ...
        try:
            ...
            stats['untapped'] = untapped_count  # Alias
        except:
            pass
        
        return jsonify({
            'success': True,
            **stats  # Flatten stats for easier access
        })
