...
# =============================================================================
# GMAIL AUTO-DETECT SALES INTEGRATION
# =============================================================================

@app.route('/ebay/api/gmail/status', methods=['GET'])
@ebay_login_required
def gmail_status():
    """Check if Gmail is connected for the current team."""
    team = get_current_team()
    return jsonify({
        'connected': bool(team.gmail_tokens_json),
        'last_sync': team.last_gmail_sync.isoformat() if hasattr(team, 'last_gmail_sync') and team.last_gmail_sync else None
    })

@app.route('/ebay/api/gmail/auth', methods=['GET'])
@ebay_login_required
def gmail_auth():
    """Start Google OAuth flow to connect Gmail."""
    try:
        from google_auth_oauthlib.flow import Flow
    except ImportError:
        return jsonify({'error': 'OAuth packages not installed'}), 500

    client_id = os.environ.get('GMAIL_CLIENT_ID')
    client_secret = os.environ.get('GMAIL_CLIENT_SECRET')

    if not client_id or not client_secret:
        return jsonify({'error': 'GMAIL_CLIENT_ID or GMAIL_CLIENT_SECRET map missed'}), 500

    client_config = {
        "web": {
            "client_id": client_id,
            "project_id": "ebay-sales-helper",
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
            "client_secret": client_secret,
            # We redirect back to the Render dashboard
            "redirect_uris": [request.host_url.rstrip('/') + "/ebay/api/gmail/callback"]
        }
    }
    ...
    flow = Flow.from_client_config(
        ...
        redirect_uri=request.host_url.rstrip('/') + "/ebay/api/gmail/callback"
    )

    auth_url, _ = flow.authorization_url(prompt='consent', access_type='offline')
    return jsonify({'auth_url': auth_url})

@app.route('/ebay/api/gmail/callback', methods=['GET'])
def gmail_callback():
    """Handle Google OAuth return."""
    # Since OAuth redirects drop our custom headers, we need to rely on the cookie
    token = request.cookies.get('eb_team')
    if not token:
        return "Not authenticated", 401

    try:
        # Dirty decode just to get team ID...
    ...
    try:
        ...
        return "OAuth packages missing", 500
    ...
    try:
        ...
        return redirect('/ebay/settings?gmail=connected')
    except Exception as e:
        log.error(f"Gmail connect error: {e}")
        return f"Failed to connect Gmail: {str(e)}", 500

@app.route('/ebay/api/gmail/sync', methods=['POST'])
@ebay_login_required
def gmail_sync():
    """Trigger a manual scan of Gmail inbox for eBay sales."""
    team = get_current_team()
    ...
    try:
        ...
        result = sync_ebay_sales(team.id, days_back=14)
        
        # Update last sync timestamp
        if not hasattr(team, 'last_gmail_sync'):
            # Some older database schemas might not have this column
            pass
        else:
            team.last_gmail_sync = datetime.utcnow()
            db.session.commit()
        ...
        log.error(f"Gmail sync failed: {e}")
        return jsonify({'error': str(e)}), 500
