...
# Tiny transparent PNG used as a graceful 200-response fallback when an
# upstream CDN refuses our fetch. iOS Safari is inconsistent about firing
# <img onerror> when the response is a 502 with text/html, but it reliably
# fires on an empty/invisible image — this gives the placeholder logic a
# clean hand-off.
...
def _proxy_fallback_png():
    """Return a 1x1 transparent PNG with a custom header so the frontend
    can detect the failure + swap in a placeholder."""
    from flask import Response
    resp = Response(_TRANSPARENT_1X1_PNG, content_type='image/png')
    resp.headers['Cache-Control'] = 'no-cache'
    resp.headers['X-Proxy-Status'] = 'upstream-failed'
    return resp
